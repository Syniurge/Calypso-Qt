...
namespace {
    ...
    QString toolBarAreaStringFromDOMAttributes(const D::WriteInitialization::DomPropertyMap &attributes) {
        ...
        case DomProperty::Number: {
            QString area = QLatin1String("cast(ToolBarArea) (");
        ...
        case DomProperty::Enum: {
            ...
            area.insert(0, QLatin1String("ToolBarArea."));
            area += QLatin1String(", ");
            return area;
        }
        default:
...
//         if (!sizeType.startsWith(QLatin1String("QSizePolicy::")))
    ...
    inline void openIfndef(QTextStream &str, const QString &symbol) { if (!symbol.isEmpty()) str << QLatin1String("static if (!is(typeof(") << symbol << "))) {" << endl;  }
    inline void closeIfndef(QTextStream &str, const QString &symbol) { if (!symbol.isEmpty()) str << QLatin1String("} // ") << symbol << endl; }
...
void WriteInitialization::acceptActionRef(DomActionRef *node)
{
    ...
    m_actionOut << m_indent << varName << ".QWidget.addAction(" << actionName << ");\n";
}
    ...
    for (int i=0; i<lst.size(); ++i) {
        ...
        if (isTopLevel && propertyName == QLatin1String("geometry") && p->elementRect()) {
            ...
            continue;
                    ...
                    && p->kind() == DomProperty::String) {
            // Workaround for D which doesn't do implicit ctor calls
            QString value = autoTrCall(p->elementString());
            autoTrOutput(p) << m_indent << varName << ".setShortcut(QKeySequence(" << value << "));\n";
            continue;
            ...
            if (p->elementEnum() == QLatin1String("Orientation.Vertical"))
            ...
            if (!frameShadowEncountered)
                m_output << m_indent << varName << ".setFrameShadow(QFrame.Shadow.Sunken);\n";
            continue;
        ...
        case DomProperty::CursorShape:
            ...
            propertyValue = QString::fromLatin1("QCursor(CursorShape.%1)")
...
//             }
        ...
        case DomProperty::Locale: {
             ...
             propertyValue = QString::fromLatin1("QLocale(QLocale.Language.%1, QLocale.Country.%2)")
...
QString  WriteInitialization::writeSizePolicy(const DomSizePolicy *sp)
{
    ...
    do {
        if (sp->hasElementHSizeType() && sp->hasElementVSizeType()) {
            m_output << " = QSizePolicy(cast(QSizePolicy.Policy) (" << sp->elementHSizeType()
                << "), cast(QSizePolicy.Policy) (" << sp->elementVSizeType() << "));\n";
            break;
        }
        if (sp->hasAttributeHSizeType() && sp->hasAttributeVSizeType()) {
                m_output << " = QSizePolicy(QSizePolicy.Policy." << sp->attributeHSizeType() << ", QSizePolicy.Policy."
                              ...
                              const DomResourceIcon *i)
{
    if (i->hasElementNormalOff())
        output << indent << iconName << ".addFile(" << writeString(i->elementNormalOff()->text(), indent) << ", QSize(), QIcon.Mode.Normal, QIcon.State.Off);\n";
    if (i->hasElementNormalOn())
        output << indent << iconName << ".addFile(" << writeString(i->elementNormalOn()->text(), indent) << ", QSize(), QIcon.Mode.Normal, QIcon.State.On);\n";
    if (i->hasElementDisabledOff())
        output << indent << iconName << ".addFile(" << writeString(i->elementDisabledOff()->text(), indent) << ", QSize(), QIcon.Mode.Disabled, QIcon.State.Off);\n";
    if (i->hasElementDisabledOn())
        output << indent << iconName << ".addFile(" << writeString(i->elementDisabledOn()->text(), indent) << ", QSize(), QIcon.Mode.Disabled, QIcon.State.On);\n";
    if (i->hasElementActiveOff())
        output << indent << iconName << ".addFile(" << writeString(i->elementActiveOff()->text(), indent) << ", QSize(), QIcon.Mode.Active, QIcon.State.Off);\n";
    if (i->hasElementActiveOn())
        output << indent << iconName << ".addFile(" << writeString(i->elementActiveOn()->text(), indent) << ", QSize(), QIcon.Mode.Active, QIcon.State.On);\n";
    if (i->hasElementSelectedOff())
        output << indent << iconName << ".addFile(" << writeString(i->elementSelectedOff()->text(), indent) << ", QSize(), QIcon.Mode.Selected, QIcon.State.Off);\n";
    if (i->hasElementSelectedOn())
        output << indent << iconName << ".addFile(" << writeString(i->elementSelectedOn()->text(), indent) << ", QSize(), QIcon.Mode.Selected, QIcon.State.On);\n";
}
...
QString WriteInitialization::trCall(const QString &str, const QString &commentHint) const
{
    ...
    QString result;
    const QString comment = commentHint.isEmpty() ? QLatin1String("null") : fixString(commentHint, m_dindent);
