...
static inline QString moduleHeader(const QString &, const QString &header)
{
//     QString rc = module;
//     rc += QLatin1Char('/');
//     rc += header;
...
void WriteIncludes::acceptUI(DomUI *node)
{
    ...
    m_output << "modmap (C++) \"<QtWidgets>\";" << QLatin1Char('\n') << QLatin1Char('\n');

    // Group common class imports
    ...
    // Qt:: enums
    m_output << "import (C++) Qt.Orientation, Qt.CursorShape, Qt.ToolBarArea, Qt.DockWidgetArea;" << QLatin1Char('\n');
    m_output << QLatin1Char('\n');

    // QStringLiteral macro replacement
    m_output << "private alias QStringLiteral = QString;" << QLatin1Char('\n');
    m_output << QLatin1Char('\n');
...
//     // Workarounds for implicit constructors calls using UFCS
//     m_output << "private void setShortcut(ref QAction a, const scope ref QString shortcut) {" << QLatin1Char('\n')
//             << "    a.setShortcut(QKeySequence(shortcut));" << QLatin1Char('\n')
//             << "}" << QLatin1Char('\n');
//     m_output << QLatin1Char('\n');
}
...
void WriteIncludes::acceptProperty(DomProperty *node)
{
    ...
    if (node->kind() == DomProperty::Enum || node->kind() == DomProperty::Set) {
        ...
        if (propertyValue.count(dotChar)) {
            bool isClass = propertyValue.count(dotChar) == 2;

            QString tagName = propertyValue.left(propertyValue.indexOf(dotChar));
            QString moduleName = tagName;
            if (!isClass)
                moduleName.insert(0, QLatin1String("Qt."));

            add(tagName, true, moduleName, true);
        }
    }
...
//     const QChar closingQuote = global ? QLatin1Char('>') : QLatin1Char('"');
    ...
    for (OrderedSet::const_iterator sit = headers.constBegin(); sit != cend; ++sit) {
        ...
        if (!header.trimmed().isEmpty()) {
            m_output << (global ? "import (C++) " : "import ") << header << ";" << QLatin1Char('\n');
        }
    }
}
